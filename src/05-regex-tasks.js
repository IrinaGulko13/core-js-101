...
function getRegexForGuid() {
  const reg = new RegExp(/^[{]{1}[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}[}]{1}$/i);
  return reg;
}
...
function getPasswordValidator(minLength) {
  const reg = new RegExp(`^((?=\\w*[a-z])(?=\\w*[A-Z])(?=\\w*[0-9])[a-zA-Z0-9]{${minLength},})$`);
  return reg;
}
